...
export default class TreeWidget {
    ...
    constructor(config) {
        this._config = {
            ...
            getNodeStyle: config.getNodeStyle || Function.prototype,
            getLinkStyle: config.getLinkStyle || Function.prototype,
            data: config.data,
    ...
    drawNodes(nodes, mainGrp) {
        ...
        var self = this,
            node = mainGrp.selectAll(".node")
            .data(nodes.descendants())
            .enter().append("g")
            .attr("class", function (d) {
                var additionalStyle = self._config.getNodeStyle(d);
                return "node " + (additionalStyle ? additionalStyle : "");
            })
    ...
    drawLinks(nodes, mainGrp) {
        ...
        var self = this,
            link = mainGrp.selectAll(".link")
            .data(nodes.descendants().slice(1))
            .enter().append("path")
            .attr("class", function (d) {
                var additionalStyle = self._config.getLinkStyle(d);
                return "link " + (additionalStyle ? additionalStyle : "");
            })
            .attr("d", function (d) {
                return "M" + d.x + "," + d.y +
